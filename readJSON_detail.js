$(function(){
	$("#main_content").ready(function(){
		...
		var url = "https://infosys-projecta-2021.github.io/2021-project2-g/docs/data.json";
		...
		$.getJSON(url,function(data){ //urlの文字列のURLからidまたはqで指定した値を持つJSONオブジェクトを取得
			...
			if($("main").get(0)){ //<main>～</main>があったら HTMLを生成
				...
				if($("p#detail2").get(0)){//<p id="detail2"></p>のタグがあったら
					$("p#detail2").html(obj.detail2); //detail2（DBではカラム）の値を変更
				}
				if($("p#detail3").get(0)){//<p id="detail3"></p>のタグがあったら
					$("p#detail3").html(obj.detail3); //detail3（DBではカラム）の値を変更
				}
				if($("p#detail4").get(0)){//<p id="detail4"></p>のタグがあったら
					$("p#detail4").html(obj.detail4); //detail4（DBではカラム）の値を変更
				}
				if($("p#location").get(0)){//<p id="location"></p>のタグがあったら
					$("p#location").html(obj.location); //location（DBではカラム）の値を変更
				}
